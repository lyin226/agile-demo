...
3、下载redis，启动redis服务。
3、安装nginx，根据自己的电脑系统，上网查找，我的电脑是mac的，从网上找了一个安装即可；修改nginx下的配置，打开nginx目录，/nginx/nginx.conf
...
根据自己的电脑中的agile-demo-ui的目录，配置nginx指定的首页地址和后端服务的地址。
5、注意nginx中的前端配置指定前端login.html文件的路径、后端服务url和应用保持一致，
源码项目中的mysql配置、redis配置改为本地服务地址，启动后端应用程序。
